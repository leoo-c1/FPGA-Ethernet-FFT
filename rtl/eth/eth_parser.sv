...
module eth_parser #(
    ...
    output logic payload_flush          // NEW: Tells the FIFO to drop the frame on sequence error
    );
    ...
    (* keep *) eth_states state;        // The current ethernet state we are in
    ...
    logic ignore_payload;               // Flags whether to drop incoming packet
    ...
    always_ff @ (posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            // Force idle if we aren't receiving valid data
            state <= IDLE;
            payload_valid <= 1'b0;
            payload_last <= 1'b0;
            payload_flush <= 1'b0;
            ...
            if (state == IDLE) begin
                ...
                if (byte_valid) begin
                    if (byte_counter == 0) begin
                        // INTERCEPT SEQUENCE ID: Do not assert payload_valid
                        payload_valid <= 1'b0;
                        if (received_byte == 8'h01) begin
                            if (expected_seq == 8'h02) begin
                                payload_flush <= 1'b1; // Missed a 0x02, flush the old 0x01 data
                            end
                            expected_seq <= 8'h02;
                            ignore_payload <= 1'b0;
                        end else if (received_byte == 8'h02) begin
                            if (expected_seq == 8'h02) begin
                                expected_seq <= 8'h01; // Perfect sequence
                                ignore_payload <= 1'b0;
                            end else begin
                                ignore_payload <= 1'b1; // Missed a 0x01, drop this 0x02
                            end
                        end else begin
                            ignore_payload <= 1'b1; // Invalid sequence ID
                        end
                        byte_counter <= byte_counter + 1'b1; // Move to actual payload data
                    end else begin
                        // ACTUAL AUDIO DATA: Send to FIFO
                        payload_flush <= 1'b0; // Deassert flush flag
                        ...
                        if (byte_counter < udp_len_flat - 16'd9) begin
                            byte_counter <= byte_counter + 1'b1;
                            payload_last <= 1'b0;
                        end else if (byte_counter == udp_len_flat - 16'd9) begin
                            byte_counter <= 0;
                ...
                // Clear payload_last when we enter FCS
                payload_last <= 1'b0;
                payload_valid <= 1'b0;
            ...
            end else if (state == DROP_FRAME) begin
                // Ignore frame
                payload_valid <= 1'b0;
                payload_last <= 1'b0;
